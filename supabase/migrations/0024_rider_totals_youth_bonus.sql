-- Add stage_youth_bonus to v_rider_totals.
--
-- v_team_stage_points (migration 0021) already includes youth bonus for team
-- scores but v_rider_totals was only summing stage_point_table values. This
-- means the Riders tab showed wrong totals for youth-bonus recipients.
--
-- stage_points in v_rider_totals now = position points + youth bonus (same
-- basis as team scoring). The per-stage breakdown columns (from
-- v_rider_stage_points) still show position-only points; only the Total column
-- includes the bonus. A future migration can surface per-stage youth bonus if
-- needed.
...
create view public.v_rider_totals as
  ...
  youth_pts as (
    select pool_id, rider_id, sum(bonus_points) as youth_points
    from   public.stage_youth_bonus
    group  by pool_id, rider_id
  ),
  gc_pts as (
  ...
  matched as (
    select
      ...
      r.bib_number,
      r.pcs_slug,
      -- stage_points includes youth bonus (same basis as v_team_stage_points)
      coalesce(s.stage_points, 0)
        + coalesce(y.youth_points, 0)                    as stage_points,
      coalesce(g.gc_points, 0)                           as gc_points,
      coalesce(s.stage_points, 0)
        + coalesce(y.youth_points, 0)
    ...
    left join youth_pts  y  on y.pool_id  = r.pool_id and y.rider_id  = r.id

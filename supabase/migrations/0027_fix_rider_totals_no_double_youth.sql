-- Migration 0026 added youth bonus to v_rider_stage_points (finish + youth
-- per stage row). But v_rider_totals (0024) was already adding youth bonus
-- separately from stage_youth_bonus, so after 0026 youth was counted twice.
--
-- Fix: remove the separate youth_pts CTE and the y.youth_points addition.
-- v_rider_totals.stage_points now = sum(v_rider_stage_points.points)
-- which already includes finish + youth on each row. One source of truth.
